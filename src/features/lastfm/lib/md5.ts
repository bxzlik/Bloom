/**
 * Компактная реализация MD5 (hex digest для UTF-8 строки).
 * Нужна только для подписи запросов Last.fm (`api_sig = md5(sorted_params + secret)`).
 * Самодостаточна, без зависимостей.
 *
 * Алгоритм — классический (RFC 1321), стандартная битовая реализация.
 */
...
function str2blks(str: string): number[] {
  ...
  for (let i = 0; i < str.length * 8; i += 8) {
    blks[i >> 5] = (blks[i >> 5] || 0) | ((str.charCodeAt(i / 8) & 0xff) << i % 32)
  }
  return blks
}
...
/** MD5 hex-дайджест UTF-8 строки. */
export function md5(input: string): string {
  // UTF-8: каждый символ → байты (Last.fm подписывает UTF-8).
  const utf8 = unescape(encodeURIComponent(input))
  return binl2hex(coreMd5(str2blks(utf8), utf8.length * 8))
}
