 ...
 *
 * Кеш в `localStorage['bloom_artist_avas']`, TTL 30 дней. Запоминается только
 * ВНЯТНЫЙ ответ площадки — в том числе «такого артиста нет» (пустой url), чтобы
 * не дёргать сеть повторно. Сорванный запрос (нет связи, площадка отказала) в
 * кеш не идёт: он переживает перезапуск, и артист, впервые спрошенный в
 * офлайне, иначе остался бы без аватарки на все 30 дней. Площадка выключена или
 * не настроена — тоже тихо игнорируем: в топе остаётся обложка лучшего трека
 * артиста, а включат — спросим.
 */
    ...
    void (async () => {
      ...
      for (const r of todo) {
        ...
        try {
          ...
          // До площадки не достучались — это НЕ ответ «такого артиста нет», и
          // запоминать его нельзя: кеш переживает перезапуск, и артист, впервые
          // спрошенный в офлайне, остался бы без аватарки на все 30 дней. Со
          // связью попытка повторится (порт `findArtistCached` с мобилки).
          inFlight.delete(k)
          continue
        }
        // Сюда доходит только внятный ответ площадки, в том числе «не нашли»
        // (url пустой) — вот его и запоминаем, чтобы не спрашивать заново.
        c[k] = { url, t: Date.now() }
        inFlight.delete(k)
        changed = true
      }
